...
bool Buddy::capable(Activity<Workout> workout_activity)
{
	...
	if (this->getEnergy() + workout_activity.item.getEnergyChange() < 0|| this->fat + workout_activity.item.getFatChange() < 0 ||this->muscle + workout_activity.item.getMuscleChange() < 0){
...
bool Buddy::capable(Activity<Dining> dining_activity) {
     ...
     * Add your code here!
     * */
	if (this->getMoney() - dining_activity.item.getPrice() < 0){
		return false;
	}
	else if(dining_schedule.isFree(dining_activity.start_time,dining_activity.end_time)){
...
Buddy& Buddy::operator<<(Activity<Workout> workout_activity) {
	...
	if (this->capable(workout_activity)){
		...
		this->energy = this->getEnergy() + workout_activity.item.getEnergyChange();
		this->fat = this->getFat() + workout_activity.item.getFatChange();
		this->muscle = this->getMuscle() + workout_activity.item.getMuscleChange();
		return *this;
	}
	else{
		return *this;
...
Buddy& Buddy::operator<<(Activity<Dining> dining_activity) {
	...
	if (this->capable(dining_activity)){
		...
		this->energy = this->getEnergy() + dining_activity.item.getEnergy();
		this->fat = this->getFat() + dining_activity.item.getFat();
		this->money = this->getMoney() - dining_activity.item.getPrice();
		return *this;
	}
	else{
		return *this;
